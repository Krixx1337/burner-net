...
The project file is intended to track BurnerNet's current implementation source set. For an exact file list in any given revision, treat the root [`CMakeLists.txt`](/mnt/e/Projects/CPP/burner-net/CMakeLists.txt) as the authoritative source of truth.
...
3. Select the architecture/configuration that matches your curl installation.
...
- If you add BurnerNet source files manually to another Visual Studio project, keep the file list aligned with the root [`CMakeLists.txt`](/mnt/e/Projects/CPP/burner-net/CMakeLists.txt) source list instead of relying on older snapshots or partial file lists.
